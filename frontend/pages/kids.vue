...
const kids = ref<Kid[]>([{ id: 1, firstName: "kid-1", lastName: "kid-2", userId: 4, creches: [] }]);
</script>
...
<template>
  <div class="max-w-[1400px] mx-auto mt-24 px-4">
    ...
    <table class="mt-4 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead v-if="kids.length > 0" class="text-xs text-gray-700 uppercase dark:bg-gray-700 dark:text-gray-400">
        <tr>
          ...
          <th scope="col" class="py-3">Crèches</th>
      ...
      <tbody>
        <tr v-for="kid in kids" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
          ...
          <td class="py-4">1,2,3,4</td>
          <td class="py-4">{{ kid.userId }}</td>
    ...
    <div v-if="kids.length > 5" class="flex flex-col items-center mt-12">
